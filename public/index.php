...
// Under PHP's built-in server (php -S ... public/index.php), serve an existing static
// file directly instead of routing it through the app — mirrors Apache/nginx in
// production so /storage/* uploads, CSS and images resolve during local dev too.
// This branch only runs under the cli-server SAPI; it is inert behind a real web server.
// PHP built-in sunucusunda (php -S ... public/index.php) var olan statik bir dosyayı
// uygulamadan geçirmeden doğrudan servis et — production'daki Apache/nginx davranışını
// taklit eder; böylece /storage/* yüklemeleri, CSS ve görseller local geliştirmede de
// çözülür. Bu blok yalnızca cli-server SAPI'de çalışır; gerçek web sunucusunda etkisizdir.
if (PHP_SAPI === 'cli-server') {
    $staticFile = __DIR__.'/'.ltrim((string) parse_url($_SERVER['REQUEST_URI'] ?? '/', PHP_URL_PATH), '/');
