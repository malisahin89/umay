...
<section class="relative">
    ...
    <div class="mx-auto max-w-4xl px-6 pb-4 pt-20 text-center sm:pt-24">
        <nav class="flex items-center justify-center gap-1.5 text-xs font-medium text-zinc-400">
            ...
            <i class="fa-solid fa-angle-right text-[9px]"></i><span class="text-zinc-500"><?= $this->e(\App\Support\Lang::t('nav.tags')) ?></span>
        </nav>
        <h1 class="font-display mt-6 text-4xl font-semibold tracking-tight text-zinc-900 sm:text-5xl"><?= $this->e(\App\Support\Lang::t('tags.heading')) ?></h1>
        <p class="mt-3 text-sm text-zinc-500"><?= $this->e(\App\Support\Lang::choice('count.tags', (int) count($tags))) ?></p>
    </div>
</section>
...
<section class="mx-auto max-w-4xl px-6 py-12">
    <?php if (count($tags) === 0) { ?>
        <div class="rounded-3xl border border-dashed border-zinc-300 bg-white/50 py-20 text-center text-zinc-400">
            ...
            <p class="text-sm font-medium"><?= $this->e(\App\Support\Lang::t('tags.empty')) ?></p>
        </div>
    <?php } else { ?>
